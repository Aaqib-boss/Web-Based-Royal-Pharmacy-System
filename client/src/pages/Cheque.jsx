...
const Cheque = () => {
  ...
  const fetchChequeEntries = async () => {
    ...
    try {
      ...
      setChequeEntries(data);
    } catch (error) {
      toast.error('Error fetching cheque records');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredChequeEntries = chequeEntries.filter((record) => {
    // 1. Company Filter
    ...
    if (searchQuery.trim() !== '') {
      ...
      const chqNo = (record.chequeNumber || '').toLowerCase();
      const bank = (record.bankName || '').toLowerCase();
  ...
  const generatePDFReport = async (action = 'download') => {
    ...
    doc.text('Cheques Clearance Report', 14, currentY);
    ...
    // Prepare table data
    const tableHeaders = [['Date', 'Invoice No', 'Pharmacy & City', 'Cheque / Bank', 'Cheque Amount']];
    const tableRows = filteredChequeEntries.map((record) => {
      ...
      const chequeBankDetails = `No. ${record.chequeNumber}\n(${record.bankName})`;
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        ...
        2: { cellWidth: 50 }, // Pharmacy & City
        3: { cellWidth: 50 }, // Cheque / Bank
    ...
    if (action === 'print') {
      ...
      const filename =
        companyFilter !== 'all'
          ? `cheques_report_${pharmaciesList.find((p) => p._id === companyFilter)?.companyName.replace(/\s+/g, '_')}.pdf`
          : 'cheques_report_all.pdf';
      doc.save(filename);
    }
  };
  ...
  const handleDownloadExcel = () => {
    ...
    if (companyFilter !== 'all' || dateFilter !== 'all') {
      ...
      row1['Cheque / Bank'] = 'Filter Period:';
      row1['Cheque Amount'] = dateRangeStr;
      dataRows.push(row1);
      dataRows.push({}); // Empty separator row
    }

    filteredChequeEntries.forEach((record) => {
      const formattedDate = new Date(record.date).toLocaleDateString('en-US', {
        day: '2-digit',
        month: 'short',
        year: 'numeric',
      ...
      dataRows.push({
        ...
        'City': record.city,
        'Cheque No': record.chequeNumber,
        'Bank Name': record.bankName,
        'Cheque Amount': record.amount,
      });
    });

    const worksheet = XLSX.utils.json_to_sheet(dataRows, {
      header: ['Date', 'Invoice No', 'Pharmacy Name', 'City', 'Cheque No', 'Bank Name', 'Cheque Amount']
    });
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Cheques Clearance Ledger');

    const filename =
      companyFilter !== 'all'
        ? `cheques_report_${pharmaciesList.find((p) => p._id === companyFilter)?.companyName.replace(/\s+/g, '_')}.xlsx`
        : 'cheques_report_all.xlsx';
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!chequeNumber) {
      toast.warning('Cheque Number is required');
      return;
    }
    if (!bankName) {
    ...
    try {
      if (isEditing) {
        const { data } = await api.put(`/cheques/${editId}`, payload);
        setChequeEntries(chequeEntries.map((c) => (c._id === editId ? data : c)));
        toast.success('Cheque entry updated successfully!');
      } else {
        const { data } = await api.post('/cheques', payload);
        setChequeEntries([data, ...chequeEntries]);
        toast.success('Cheque entry logged successfully!');
      }
      resetForm();
    } catch (error) {
  ...
  const handleEditClick = (record) => {
    ...
    setCity(record.city);
    setChequeNumber(record.chequeNumber);
  ...
  const handleDeleteSubmit = async () => {
    ...
    setLoading(true);
    try {
      await api.delete(`/cheques/${deleteTarget._id}`);
      setChequeEntries(chequeEntries.filter((c) => c._id !== deleteTarget._id));
      toast.success('Cheque record removed');
      setShowConfirmDelete(false);
      setDeleteTarget(null);
    } catch (error) {
      toast.error('Failed to remove cheque record');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 pt-8 relative z-10">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-primary to-primary-emerald bg-clip-text text-transparent">
            Cheques Clearance Management
          </h1>
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1 font-medium">
            Log, verify and track pharmacy cheque clearings and banking transaction trails
          </p>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-20 glass-panel rounded-3xl p-6 card-hover">
              <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6 flex items-center">
                ...
                <span>{isEditing ? 'Edit Cheque Log' : 'New Cheque Log'}</span>
              </h2>
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  >
                    {loading ? (
                      ...
                      <span>{isEditing ? 'Update Entry' : 'Log Cheque'}</span>
                    )}
                  </button>
                </div>
              </form>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="glass-panel rounded-3xl overflow-hidden shadow-xl w-full">
              <div className="px-6 py-5 border-b border-slate-100 dark:border-white/5 flex justify-between items-center">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Cheque Ledger Entries</h3>
                <button
                  onClick={fetchChequeEntries}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-12 gap-3 items-center w-full">
                  ...
                  <div className="relative md:col-span-4">
                    ...
                    <input
                      ...
                      placeholder="Search invoice, pharmacy, bank, cheque..."
                      autoComplete="off"
                  ...
                  <div className="md:col-span-3 flex gap-2 justify-end">
                    <button
                      ...
                      disabled={filteredChequeEntries.length === 0}
              ...
              {loading && chequeEntries.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-center border-collapse text-xs sm:text-sm cheque-table">
                    <thead>
                        ...
                        <tr key={record._id} className="hover:bg-slate-500/5 dark:hover:bg-white/5 transition-all duration-150">
                          ...
                          <td className="multiline-cell">
                            <div className="font-bold text-slate-700 dark:text-slate-300">
                              No. {record.chequeNumber}
                            </div>
                            <div className="text-slate-500 dark:text-slate-400 text-xs font-medium mt-0.5">{record.bankName}</div>
                          </td>
                          <td className="font-bold text-primary dark:text-primary-emerald">
                          ...
                          <td className="space-x-1.5 whitespace-nowrap">
                            ...
                            <button
                              ...
                              className="p-2 rounded-xl text-slate-405 hover:text-red-500 hover:bg-red-500/10 border border-transparent hover:border-red-500/10 dark:hover:bg-red-500/20 transition-all duration-200"
                              title="Delete"
                            >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="glass-panel w-full max-w-md rounded-3xl shadow-2xl p-6 sm:p-8 text-center animate-in zoom-in-95 duration-200">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
              You are about to delete Cheque entry for invoice{' '}
